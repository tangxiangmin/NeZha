...
const frameLength: timestamp = 1000 / 30

let frameDeadline: timestamp // 当前切片执行时间deadline，在每次执行新的切片时会更新为getCurrentTime() + frameLength
...
function onAnimationFrame() {
    ...
    const hasMoreWork: boolean = scheduledCallback();
...
function onIdleFrame(deadline) {
    if (!scheduledCallback) return;
    let remain: timestamp = deadline.timeRemaining()// 当前帧剩余可用的空闲时间
